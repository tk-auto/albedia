---
title: Diamond River
tags:
  - locations
  - landmark
---

# Diamond River

The Diamond River is a very broad, many-branched river of
[Hereterra](hereterra.md), in [Albedia](../index.md), rising in
the [Barren Peaks](barren-peaks.md) and running west to spill into the
northern head of [Val Verde](val-verde.md).

## Geography

The river takes its source high in the [Barren Peaks](barren-peaks.md) and
descends into the lowlands, where it splits into many ramifications and broad
marshland rather than a single channel. It passes the fertile
[Esper Counties](esper-counties.md) and runs on westward, spilling at last into
the northern head of [Val Verde](val-verde.md), just below the crater of
[Col Vulca](col-vulca.md), before the valley carries it on southward. The
corrosive wind of the [Scythed Lands](scythed-lands.md) is said to climb back
up its course.

## The making of Lem

At an as-yet-unnamed lake along the river's course,
[Elina](../characters/elina.md) unknowingly quickened
[Lem](../characters/lem.md) from the peat of the shore — the beginning of the
tale of [Elina and Lem](../stories/elina-and-lem.md).

## Nerem's Trail

Because trade is frequent between the [Esper Counties](esper-counties.md) and the
[Taje Plateaus](taje-plateaus.md), the overland route known as **Nerem's Trail** —
named after [Nerem](../characters/nerem.md), who first found the way — has been
extensively improved to carry the convoys that pass along it.

## See also

- [Hereterra](hereterra.md) — the region it flows through.
- [Barren Peaks](barren-peaks.md) — its source.
- [Val Verde](val-verde.md) — the valley it empties into.
- [Esper Counties](esper-counties.md) and [Taje Plateaus](taje-plateaus.md) — the
  lands its trail connects.
- [Nerem](../characters/nerem.md) — namesake of the trail.
- [Lem](../characters/lem.md) and [Elina](../characters/elina.md) — made and
  maker, at a lake along the river.
