---
title: Glossary
tags:
  - glossary
---

# Glossary

Terms particular to the world of [Albedia](index.md). Each entry gives a short
definition and points to the fuller account in the relevant
[location](locations/index.md) or [people](peoples/index.md) article.

Anchor
:   A fixing point where a hanging city's structures are made fast to the
    underside of a Roc, cunningly set into the stone's natural irregularities and
    constantly readjusted as the suspended weight shifts. See
    [The SkyHangers](peoples/the-skyhangers.md#terminology).

Courser
:   A nimble courier who carries a delivery from one point to another by running
    and vaulting through the structures of a city. See
    [The SkyHangers](peoples/the-skyhangers.md#terminology).

Glider
:   Both the limited-range flying wing used to travel between the cities of the
    Rocs and the pilot who flies it. See
    [The SkyHangers](peoples/the-skyhangers.md#terminology).

Life Components
:   The six elements — Earth, Water, Air, Fire, Soul, and Malia — whose
    interaction composes life. See
    [The Life Components](myths/the-life-components.md).

Malia
:   The sixth Life Component, the binder that joins the other five so that they
    create Life; some individuals can wield it directly. See
    [The Life Components](myths/the-life-components.md#malia).

Roc
:   One of the great floating, motionless masses of stone from which the cities
    of the [Mistness](locations/mistness.md) hang — its stone unpiercable and
    immune to erosion, fully drained of Malia and so unable to move. See
    [Mistness](locations/mistness.md),
    [The SkyHangers](peoples/the-skyhangers.md#terminology), and
    [The Life Components](myths/the-life-components.md#even-stone-is-a-little-alive).

## See also

- [The SkyHangers](peoples/the-skyhangers.md) — the people whose way of life
  gives most of these terms.
- [Mistness](locations/mistness.md) — the clouded pole and its Rocs.
- [The Life Components](myths/the-life-components.md) — the cosmology behind
  *Life Components* and *Malia*.
